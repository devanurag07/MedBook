...
class SmartTable extends Component {
    ...
    constructURL(offset, end,filterParams) {
        ...
        if (this.props.defaultParam) {
            Object.keys(this.props.defaultParam).forEach(key => {
                url += `${encodeURIComponent(key)}=${encodeURIComponent(this.props.defaultParam[key])}&`;
            })
        }
        if (this.filterParams) {
            Object.keys(this.filterParams).forEach(key => {
                url += `${encodeURIComponent(key)}=${encodeURIComponent(this.filterParams[key])}&`;
            })
        }
    ...
    _switchPart(column, data) {
        switch (column.type) {
            case 'action': {
                return <ActionTemplate data={data} actionHandler={this.props.actionHandler} column={column} />;
                break;
            }
            case 'lookup_badge': {
                ...
                let badgeClassName = itemValue === 'Open' ? 'badge bg-danger' : 'badge bg-success'
                return <BadgeTemplate data={itemValue} badgeClassName={badgeClassName} />;
                break;
            }
            case 'date':
            ...
            case 'file':
                ...
                if (urlData) {
                    return (
                        <a href={urlData} target="_blank" download>Download</a>
                    )
                }
                else {
                    return  ''
    ...
    _switchColFilterPart(column) {
        ...
        switch (column.filterConfig) {
            case 'select':
                elementConfig = { options: column.lookupData, filterLabel: column.filterLabel, optionKey: column.objectKey };
                return <Input
                    elementType='select'
    ...
    changePage(page, pageSize) {
        ...
        let startVal = (page - 1) * parseInt(pageSize ? pageSize : this.state.pageSize);
                        ...
                        <Table striped responsive>
                            ...
                            <tbody>
                                {this.props.colFilter &&
                                    <tr>{this.state.cols.map((column, i) =>
                                        <td key={i}>
                                            {this._switchColFilterPart(column)}
                                        </td>
                                    )}</tr>
                                }
                                ...
                                {this.state.rows.length == 0 &&
                                    <tr><td className="text-center" colSpan={this.props.cols.length + 1}>No records</td></tr>
                                }
                            </tbody>
                        </Table>
                    </div>
                ...
                <div className="row">
                    <div className="col-2">
                        <div className="form-group row">
                            <label className="col-sm-4 col-form-label col-form-label-sm">Size</label>
                            <div className="col-sm-8">
                                <BootstrapInput type="select" className="sm-drop-down bs" value={this.state.pageSize} onChange={this.changeLimit.bind(this)} name="pageSize" bsSize="sm">
                                    {this.pageSizeOptions.map((s, i) => <option key={i} value={s}>{s}</option>)}
                                </BootstrapInput>
                            </div>
                        </div>
                    </div>
